...
test("blog index lists all fourteen posts newest first", async ({ page }) => {
  ...
  expect(titles[0]).toMatch(/Test Commander after Phase 4/i);
...
test("feed.json validates as JSON Feed 1.1 and lists the posts", async ({ page }) => {
  ...
  expect(body.items[0].url).toContain("/blog/test-commander-after-phase-4-hands-on-tour/");
});
