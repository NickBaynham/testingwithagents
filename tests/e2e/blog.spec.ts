...
test("blog index lists all thirteen posts newest first", async ({ page }) => {
  ...
  expect(titles[0]).toMatch(/Test Commander Phase 3/i);
...
test("feed.json validates as JSON Feed 1.1 and lists the posts", async ({ page }) => {
  ...
  expect(body.items[0].url).toContain(
    "/blog/test-commander-phase-3-five-helpers-shared-synthesizer/",
  );
});
