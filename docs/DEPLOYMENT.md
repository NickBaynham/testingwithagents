...
| `post-deploy-smoke` job exits with `Timed out waiting 120000ms from config.webServer` | `playwright.config.ts` unconditionally declared a `webServer` block that runs `serve out` on port 3000. The smoke job sets `LIVE_URL` but never downloads the `next-build` artifact, so there is no `out/` directory to serve; Playwright waits 120s for `BASE_URL` to come up and times out before any spec runs. | Gate the `webServer` block on `process.env.LIVE_URL` being unset. When `LIVE_URL` is set, Playwright skips the local server and the specs hit the deployed URL directly (which is what they're written to do via absolute URLs). | `playwright.config.ts` (conditional `webServer`) |
| Entire domain returns NXDOMAIN (browser shows "site can't be reached" / 404); Amplify URL still serves 200 | ICANN registrant-email verification was never completed. Route 53 Domains sends a verification email to the registrant contact at registration; if the link is not clicked within 15 days, the registrar sets `clientHold` and the registry pulls the domain from the `.com` zone. Diagnose with `whois testingwithagents.com \| grep -i status` (shows `clientHold`) and `aws route53domains get-contact-reachability-status --domain-name testingwithagents.com --region us-east-1` (shows `EXPIRED` or `PENDING`). | Resend the email with `aws route53domains resend-contact-reachability-email --domain-name testingwithagents.com --region us-east-1`, then click the verification link sent to the registrant contact (currently `info@calgentik.com`). The hold lifts automatically after verification; DNS returns within minutes to a few hours. Prevent recurrence: verify immediately whenever the registrant contact email changes, and keep the registrant mailbox monitored. | Manual: reachability status `DONE` + `dig +short testingwithagents.com` resolving |
